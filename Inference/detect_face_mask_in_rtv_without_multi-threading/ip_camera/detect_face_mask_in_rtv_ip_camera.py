...
cap = cv2.VideoCapture(srcURL)
fps = FPS().start() #Notes the start time
width = 440
with open("benchmarking_without_multithreading_ip_camera.csv", 'w', newline='') as file:
	writer = csv.writer(file)
	writer.writerow(["Thread Frame #",  "Time spent in reading the frame from ip camera (seconds)", "Time spent performing inference on the frame (seconds)"])
